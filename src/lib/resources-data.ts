// Single source of truth for the /resources hub + dynamic article pages.
// Articles are operator-written: real numbers, real tactics, real construction context.

export type ResourceCategory = "Guide" | "Playbook" | "Case Study" | "Calculator";

export type ArticleSection = {
  id: string; // anchor slug for TOC
  heading: string;
  // Paragraphs of body text — paragraphs render as <p>. To insert a pull quote,
  // use a paragraph that starts with the literal token "PULLQUOTE::" — the
  // ArticleContent component will render the rest as a styled blockquote.
  paragraphs: string[];
};
...
export type Resource = {
  ...
  title: string;
  // Short, descriptive deck shown on cards + at the top of the article
  dek: string;
  readMinutes: number;
  publishedISO: string; // YYYY-MM-DD
  author: { name: string; role: string };
  // Used by the dynamic /resources/[slug] page. If `externalAnchor` is set,
  // the hub card links there instead of to /resources/[slug] (e.g. calculator).
  externalAnchor?: string;
  // For card preview
  preview: string;
  // Article body (omitted only when externalAnchor is used)
  sections?: ArticleSection[];
  // Related resource slugs (max 3)
  related?: string[];
};

export const RESOURCES: Resource[] = [
  /* ────────────────────────────────────────────────────────────────
   * 1. Guide — speed-to-lead in construction
   * ──────────────────────────────────────────────────────────────── */
  {
    slug: "hidden-cost-of-slow-lead-response",
    category: "Guide",
    title: "The hidden cost of slow lead response in construction",
    dek: "Why a 5-minute callback isn't fast — and what the actual math says about every minute past 90 seconds.",
    readMinutes: 9,
    publishedISO: "2026-04-22",
    author: { name: "Yuval Ben-Rashi", role: "Operating Partner, APEX" },
    preview:
      "The construction industry has accepted 'we'll call you back within an hour' as normal. It is not normal. It is the largest hidden revenue leak in your business, and the math is brutal.",
    related: ["audit-your-no-show-rate", "true-revenue-leak-calculator"],
    sections: [
      {
        id: "the-90-second-rule",
        heading: "The 90-second rule nobody respects",
        paragraphs: [
          ...
          "PULLQUOTE::Past 90 seconds, contact rate degrades faster than past 5 minutes — because the homeowner has already clicked the next ad.",
        ],
      },
      {
        id: "the-math",
        heading: "The math, in dollars",
        paragraphs: [
      ...
      {
        id: "why-internal-fails",
        heading: "Why hiring an internal CSR doesn't fix it",
        paragraphs: [
      ...
      },
      {
        ...
        heading: "What actually works at scale",
        paragraphs: [
      ...
      {
        id: "what-to-audit-this-week",
        heading: "What to audit in your own business this week",
        paragraphs: [
  ...
  /* ────────────────────────────────────────────────────────────────
   * 2. Playbook — auditing your no-show rate
   * ──────────────────────────────────────────────────────────────── */
  {
    slug: "audit-your-no-show-rate",
    category: "Playbook",
    title: "How to audit your no-show rate without a consultant",
    dek: "A four-step playbook to measure, segment, and cut no-shows by 50%+ — using only your existing CRM and a spreadsheet.",
    readMinutes: 11,
    publishedISO: "2026-04-29",
    author: { name: "Yuval Ben-Rashi", role: "Operating Partner, APEX" },
    preview:
      "Every construction owner thinks they know their no-show rate. Most are wrong by 8–12 percentage points. Here is the four-step audit that gets you the real number — and the three confirmation tactics that move it 50%+ in 30 days.",
    related: ["hidden-cost-of-slow-lead-response", "true-revenue-leak-calculator"],
    sections: [
      {
        id: "why-you-dont-know-your-number",
        heading: "Why you don't actually know your no-show rate",
        paragraphs: [
      ...
      {
        id: "step-1-define-no-show",
        heading: "Step 1 — Define 'no-show' precisely",
        paragraphs: [
      ...
      {
        id: "step-2-pull-the-data",
        heading: "Step 2 — Pull the data for the last 90 days",
        paragraphs: [
          ...
          "PULLQUOTE::If your CRM can't export the actual outcome of an appointment, that is your first finding — and the only thing you need to instrument before doing anything else.",
        ],
      },
      {
        id: "step-3-segment",
        heading: "Step 3 — Segment by the four axes that matter",
        paragraphs: [
      ...
      {
        id: "step-4-the-three-tactics",
        heading: "Step 4 — Apply the three confirmation tactics",
        paragraphs: [
      ...
      },
      {
        ...
        heading: "What the numbers mean in dollars",
        paragraphs: [
      ...
      },
      {
        ...
        heading: "What most owners get wrong about no-shows",
        paragraphs: [
  ...
  /* ────────────────────────────────────────────────────────────────
   * 3. Calculator — links to home #simulator anchor
   * ──────────────────────────────────────────────────────────────── */
  {
    slug: "true-revenue-leak-calculator",
    category: "Calculator",
    title: "Calculator: Your true revenue leak per month",
    dek: "30-second diagnostic — enter your monthly lead volume, average ticket, and current close rate. See the recoverable revenue you're leaving on the table.",
    readMinutes: 1,
    publishedISO: "2026-05-01",
    author: { name: "APEX Operating Team", role: "Diagnostic" },
    externalAnchor: "/#simulator",
    preview:
      "Plug in your real numbers — lead volume, average ticket, current close rate, current no-show rate. See exactly where the revenue leak is and what's recoverable in 90 days.",
    related: ["hidden-cost-of-slow-lead-response", "audit-your-no-show-rate"],
  },
];
...
export const ARTICLE_SLUGS: string[] = RESOURCES.filter((r) => r.sections).map(
  (r) => r.slug
);
